...
import { startTenantLineLink } from "./line/actions";
import { getTenantLineConnection } from "./line/data";
...
const dateTime = (value: string) => new Intl.DateTimeFormat("ja-JP", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Tokyo" }).format(new Date(value));
...
export default async function TenantPage({ searchParams }: { searchParams: Promise<{ line?: string; lineLinked?: string; lineError?: string }> }) {
  ...
  let repairs, lineConnection;
  try { [repairs, lineConnection] = await Promise.all([getTenantRepairs(context.tenant), getTenantLineConnection(context.tenant)]); } catch { return <TenantShell><Message>{text.unavailable}</Message></TenantShell>; }
  const params = await searchParams;
  const lineStatus = params.line;
  const lineError = params.lineError === "1" ? "LINE連携を完了できませんでした。もう一度お試しください。" : lineStatus === "already-linked" ? "既にLINE連携済みです。" : lineStatus === "account" ? "有効な入居者アカウントを確認できません。" : lineStatus === "invalid" ? "LINE連携URLが正しくありません。" : lineStatus === "error" ? "LINE連携を開始できませんでした。時間をおいて再度お試しください。" : null;
  return <TenantShell><p className="mb-5 text-sm text-slate-600">{context.tenant.displayName}</p>
  <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <h2 className="font-bold text-[#0b2e59]">LINE連携</h2>
    {lineConnection ? <div className="mt-4"><p className="font-bold text-emerald-700">LINE連携済み</p><p className="mt-2 text-sm text-slate-600">連携日時：{dateTime(lineConnection.linkedAt)}</p></div> : <><p className="mt-3 text-sm leading-6 text-slate-600">LINEと連携すると、修理の進捗確認や管理会社とのメッセージをLINEから利用できるようになります。</p><form action={startTenantLineLink} className="mt-4"><button type="submit" className="w-full rounded-xl bg-[#06c755] px-4 py-3 text-sm font-bold text-white">LINEと連携する</button></form></>}
    {params.lineLinked === "1" && !lineError && <p role="status" className="mt-3 text-sm text-emerald-700">LINE連携が完了しました</p>}
    {lineError && <p role="alert" className="mt-3 text-sm text-red-700">{lineError}</p>}
  </section>
  {repairs.length === 0 ? <Message>{text.empty}</Message> : <div className="space-y-4">{repairs.map((repair) => <article key={repair.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-l-4 border-[#b99452] p-5">
