...
export default function CalendarEventsList({events,now,canUpdate,showDate=false,onEvent}: {events:CalendarEvent[];now:number;canUpdate:boolean;showDate?:boolean;onEvent?:(event:CalendarEvent)=>void}) {
  ...
  return <><ul className="divide-y divide-slate-200">{events.map(event=><li key={event.id} className={`py-3 ${event.status==='scheduled'?'':'text-slate-500'}`}>
    <p className="break-words text-sm"><span className="mr-2 font-semibold">{showDate&&`${japanDay(Date.parse(event.starts_at))} `}{eventTime(event)}</span>
      {onEvent?<button type="button" onClick={()=>onEvent(event)} className="text-left text-[#0b2e59] underline">{event.title}</button>:event.repair_request_id ? <a className={event.status==='scheduled'?"text-[#0b2e59] underline":"text-slate-400 underline"} href={`/admin#repair-detail-${event.repair_request_id}`}>{event.title}</a>:event.title}
    ...
    {!onEvent&&canUpdate&&event.status==='scheduled'&&<div className="mt-2 flex flex-wrap items-start gap-2">
      ...
      {<CalendarEventForm repairId={event.repair_request_id} dispatchId={event.vendor_dispatch_id} event={event}/>}
    </div>}
