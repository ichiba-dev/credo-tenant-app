...
export const RepairTabContext = createContext<(tab: Tab) => void>(() => {});
export default function RepairDetailTabs({ repairId, active, desktop, sections }: {
  ...
  return <RepairTabContext.Provider value={key=>{select(key);document.getElementById(`repair-${repairId}-tab-${key}`)?.focus();}}><div className="min-w-0 break-words [&_input]:min-w-0 [&_input]:max-w-full [&_select]:max-w-full">
