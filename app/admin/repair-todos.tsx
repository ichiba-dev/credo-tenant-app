  ...
  return <section aria-labelledby="repair-todos-heading" className="mb-6 rounded-xl border border-slate-200 bg-white p-4 xl:mb-3 xl:p-3">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      {[{ key: "all", label: "すべて" }, ...TODO_RULES].map(rule => <button key={rule.key} type="button"
        aria-pressed={filter === rule.key} onClick={() => { setFilter(rule.key as TodoKey | "all"); setExpanded(false); }}
        className={`rounded-lg border px-3 py-2 text-xs ${filter === rule.key ? "bg-[#0b2e59] text-white" : "border-slate-200 text-[#0b2e59]"}`}>
        {rule.label} {rule.key === "all" ? todos.length : todos.filter(t => t.primary.key === rule.key).length}
      </button>)}
    </div>
