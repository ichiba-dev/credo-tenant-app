...
async function authorizedMessage(messageId: unknown) {
  ...
  const { data: message, error } = await db.from("tenant_line_messages")
    .select("id, organization_id, tenant_account_id, repair_request_id, channel, sender_type")
  ...
  if (error || !message || message.id !== messageId || message.organization_id !== context.organizationId ||
    !message.tenant_account_id || message.channel !== "line" || message.sender_type !== "tenant") return { failure: invalid };
  if (message.repair_request_id !== null) return { failure: assigned };
  return { context, db, signal, message };
}

export async function getLineRepairCandidates(messageId: unknown): Promise<Failure | { ok: true; repairs: LineRepairCandidate[] }> {
  try {
    const scope = await authorizedMessage(messageId);
...
export async function assignLineMessage(messageId: unknown, repairIdValue: unknown): Promise<Failure | { ok: true }> {
  try {
    const scope = await authorizedMessage(messageId);
    if (scope.failure) return scope.failure;
    ...
    const { data: updated, error: updateError } = await db.from("tenant_line_messages")
      .update({ repair_request_id: repairId }).eq("id", message.id)
      .eq("organization_id", context.organizationId).eq("tenant_account_id", message.tenant_account_id)
      .eq("channel", "line").eq("sender_type", "tenant").is("repair_request_id", null)
