...
function EventBlock({event, onSelect}: {event: CalendarEvent; onSelect: () => void}) {
  ...
  return event.repair_request_id ? <a href={`/admin#repair-detail-${event.repair_request_id}`} className={className} title={label} aria-label={label}>{content}</a> :
    <button type="button" className={`${className} w-full`} title={label} aria-label={label} onClick={onSelect}>{content}</button>;
}

export default function CalendarTimeline({events, days, day, now, onSelect}: {
  events: CalendarEvent[]; days: string[]; day: string; now: number; onSelect: (day: string) => void;
}) {
    ...
    <div className="sticky top-0 z-20 bg-white shadow-sm">
      ...
      <div className="grid border-b border-slate-200" style={columns}>
        ...
        {days.map(d => <div key={d} className={`min-w-0 space-y-1 border-l border-slate-200 p-1 ${d === today ? 'bg-blue-50/50' : ''}`}>
          {eventsOnDay(events, d).filter(e => e.all_day).map(event => <EventBlock key={event.id} event={event} onSelect={() => onSelect(d)}/>)}
        </div>)}
      </div>
    </div>
    <div className="grid pb-4" style={columns}>
      ...
      {days.map(d => <div key={d} className={`relative min-w-0 border-l border-slate-200 ${d === today ? 'bg-blue-50/40' : ''}`} style={{height}}>
        ...
        {layoutDay(events, d).map(slot => <div key={slot.event.id} className="absolute z-10 p-px hover:z-30 focus-within:z-30" style={{top: (slot.start - bounds.start) * 1.2, height: (slot.end - slot.start) * 1.2, left: `${slot.lane / slot.lanes * 100}%`, width: `${100 / slot.lanes}%`}}>
          <EventBlock event={slot.event} onSelect={() => onSelect(d)}/>
        </div>)}
