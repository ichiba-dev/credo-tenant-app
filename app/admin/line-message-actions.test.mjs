...
const message = () => ({ id, organization_id: 'org-a', tenant_account_id: 'tenant-a', repair_request_id: null, channel: 'line', sender_type: 'tenant' });
...
function setup({ role = 'admin', authenticated = true, messages = [message()], repairs = [repair()], race = false, failTable, bypass = false } = {}) {
  ...
  const db = { from(table) {
    ...
    const b = {
      ...
      then(resolve, reject) {
        ...
        let rows = table === 'tenant_line_messages' ? messages : table === 'repair_requests' ? repairs : [{ id: 'tenant-a', organization_id: 'org-a', display_name: '入居者' }];
  ...
  return { ...load('./line-message-actions.ts', imports),
    list: () => load('./line-message-data.ts', imports).getUnassignedLineMessages({ ok: true, organizationId: 'org-a' }),
...
for (const role of ['admin', 'manager', 'staff']) test(`${role} assigns same-tenant repair and message disappears from unassigned list`, async () => {
  ...
  assert.equal((await s.assignLineMessage(id, '23')).ok, true);
...
test('viewer and unauthenticated direct actions are rejected before data access', async () => {
  for (const options of [{ role: 'viewer' }, { authenticated: false }]) {
    ...
    for (const result of [await s.getLineRepairCandidates(id), await s.assignLineMessage(id, '23')]) {
...
for (const [label, options] of [
  ...
  const s = setup(options); assert.equal((await s.assignLineMessage(id, '23')).ok, false);
...
test('unexpected foreign rows are rejected even when DB filters are bypassed', async () => {
  for (const options of [{ messages: [{ ...message(), organization_id: 'org-b' }] },
    ...
    const s = setup({ ...options, bypass: true }); assert.equal((await s.assignLineMessage(id, '23')).ok, false);
    assert.equal(s.queries.some(q => q.update), false);
  }
});
test('concurrent assignment is never overwritten', async () => {
  const s = setup({ race: true }); const result = await s.assignLineMessage(id, '23');
...
test('candidates contain only same-organization same-tenant repairs and minimal display data', async () => {
  ...
  const result = await s.getLineRepairCandidates(id);
...
test('invalid IDs and DB errors never succeed or expose details', async () => {
  for (const repairId of ['0', '-1', '9007199254740992', {}, 'bad']) {
    assert.equal((await setup().assignLineMessage(id, repairId)).ok, false);
  }
  assert.equal((await setup().assignLineMessage('bad', '23')).ok, false);
  for (const failTable of ['tenant_line_messages', 'repair_requests']) {
    const result = await setup({ failTable }).assignLineMessage(id, '23');
    assert.equal(result.ok, false); assert.equal(JSON.stringify(result).includes('DB_SECRET'), false);
  }
});
test('viewer has no assignment controls; staff gets an assignment control', () => {
  ...
  for (const canUpdate of [false, true]) {
    const html = renderToStaticMarkup(jsx.jsx(Section, { messages: [{ id, tenant_name: '入居者', message: '本文', created_at: '2026-09-18T00:00:00Z' }], unavailable: false, canUpdate }));
    assert.equal(html.includes('<button'), canUpdate);
  }
});
