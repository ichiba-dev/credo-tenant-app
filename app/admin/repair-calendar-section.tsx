...
export function RepairCalendarContent({repair,events,loading,error,view='all'}: {
  repair:Pick<AdminRepair,'id'|'history'|'vendor_dispatches'>;events:RepairCalendarEvent[];loading:boolean;error:boolean;
  view?:'all'|'events'|'history'|'summary';
}) {
  const items=repairCalendarEvents(events,repair.id),timeline=repairCalendarTimeline(repair.history,items);
  const displayed=view==='summary'?items.filter(event=>event.status==='scheduled'&&Date.parse(event.ends_at??event.starts_at)>=Date.now()).slice(0,1):items;
  return <>
    {view!=='history'&&<section aria-label="案件の予定" className="mt-4 rounded-lg border border-slate-200 bg-white p-3 text-sm" aria-busy={loading}>
      <h3 className="font-bold text-[#0b2e59]">{view==='summary'?'直近の予定':'予定'}</h3>
      {loading?<p role="status">予定を確認中…</p>:error?<p role="alert">予定を取得できませんでした。案件を開き直すと再取得します。</p>:displayed.length===0?<p className="mt-2 text-slate-500">{view==='summary'?'今後の予定はありません。':'紐付いた予定はありません。'}</p>:
        <ul className="divide-y divide-slate-200">{displayed.map(event=><li key={event.id} className="space-y-1 py-3">
    ...
    {(view==='all'||view==='history')&&<section aria-label="対応履歴" className="mt-3 rounded-lg bg-slate-100 p-3 text-sm">
...
export default function RepairCalendarSection({repair,view='all',enabled=true,children}: {repair:AdminRepair;view?:'all'|'events'|'history'|'summary';enabled?:boolean;children?:(state:{events:RepairCalendarEvent[];loading:boolean;error:boolean})=>ReactNode}) {
  ...
  useEffect(()=>{
    ...
    const reopen=()=>{if(!detail||detail.open)void refresh();};
